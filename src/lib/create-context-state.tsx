...
export function createContextState<T>(initialState: T) {
  ...
  const DispatchContext = createContext<React.Dispatch<React.SetStateAction<T>>>(throwError);
  ...
  const Provider = ({ children }: React.PropsWithChildren) => {
    ...
    return (
      <StateContext.Provider value={value}>
        <DispatchContext.Provider value={setValue}>{children}</DispatchContext.Provider>
